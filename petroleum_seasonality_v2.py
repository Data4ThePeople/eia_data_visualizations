...
function startAnimation(speed) {
  ...
  // Own revision for playback: leaving it (finish/stop/any redraw) changes
  // the revision back, so the pinned range is fully discarded.
  lay.uirevision = 'replay';
  yAxisFrozen = true;
...
function layoutFor(tier) {{
  ...
  const layout = {{
    ...
    height: c.height,
    // Per-product revision: switching inventories discards the old axis
