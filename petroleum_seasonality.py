...
# DATA_DIR is the folder that contains the EIA .xls files.
# Change this if you move the data files to a different folder.
DATA_DIR = "data"

# Row offset to skip in every EIA Excel file before the real data starts.
# EIA exports always have 2 header rows — you shouldn't need to change this.
_EIA_SKIPROWS = 2


def load_eia(filename, col_name):
    """
    Reads one EIA Excel file and returns a single-column DataFrame.

    Parameters
    ----------
    filename : str
        The name of the .xls file inside DATA_DIR (e.g. "WCRSTUS1w.xls").
        Change this string if EIA renames or updates the download file.
    col_name : str
        The name to give the data column in the returned DataFrame
        (e.g. "crude_oil"). Used internally to merge and look up data.
    ...
    """
    df = pd.read_excel(
        f"{DATA_DIR}/{filename}",   # builds the full path: "data/WCRSTUS1w.xls"
        sheet_name="Data 1",        # EIA files always store data on sheet "Data 1"
        skiprows=_EIA_SKIPROWS,     # skip the two-row EIA header
        index_col=0,                # the first column is the date
        parse_dates=True,           # convert date strings to proper date objects
    )
    df.index.name = "date"
    df.columns = [col_name]         # rename the single data column
    return df


# ── Load each EIA series ───────────────────────────────────────────────────────
# Each line loads one product.
# If EIA renames a download file, update the filename string here.
# If you want to add a new product, add a new load_eia() call and include it
# in the join() below and in the PRODUCTS dict further down.
...
# ── D4TP logo ──────────────────────────────────────────────────────────────────
# Change LOGO_PATH to the location of your D4TP logo image file.
# A relative path (e.g. "d4tp_logo.png") works if the image is in the same
# folder as this script and the output HTML file.
# An absolute path (e.g. r"C:\Users\amand\Downloads\d4tp_logo.png") also works.
LOGO_PATH = "d4tp_logo.png"
...
<div class="credit-bar">
  <img src="{LOGO_PATH}" alt="D4TP" style="height:24px;width:auto;opacity:0.85;">
</div>
